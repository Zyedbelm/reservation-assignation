-- Phase 1: CORRECTION CRITIQUE DE SÉCURITÉ (corrigée)
...
CREATE POLICY "Admins can insert game masters" 
ON public.game_masters 
FOR INSERT 
WITH CHECK (EXISTS (
  SELECT 1 FROM public.profiles 
...
CREATE POLICY "Admins can update game masters" 
ON public.game_masters 
FOR UPDATE 
USING (EXISTS (
  SELECT 1 FROM public.profiles 
...
CREATE POLICY "Admins can delete game masters" 
ON public.game_masters 
FOR DELETE 
USING (EXISTS (
  SELECT 1 FROM public.profiles 
