-- Fonction pour recalculer les durées des événements basées sur les mappings de jeux
...
BEGIN
    ...
    FOR event_record IN 
        ...
        FROM activities a
        WHERE EXISTS (
            SELECT 1 FROM event_game_mappings egm 
    ...
    LOOP
        ...
        FROM games g
        JOIN event_game_mappings egm ON g.id = egm.game_id
...
-- Exécuter la fonction pour corriger toutes les durées
SELECT recalculate_event_durations();